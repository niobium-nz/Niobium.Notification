...
namespace Niobium.Notification.Host.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class SubscribeFromGoogleAdsController(Func<SubscriptionDomain> domainFactory, ILogger<SubscribeFromGoogleAdsController> logger)
        : ControllerBase
    {
        ...
        {
            GoogleAdsLeadForm request = await JsonMarshaller.UnmarshallAsync<GoogleAdsLeadForm>(this.Request.Body, JsonMarshallingFormat.SnakeCase, cancellationToken);
            if (request == null)
            {
